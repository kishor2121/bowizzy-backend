...
exports.updateSubscription = async (req, res) => {
  try {
    const user_id = req.params.user_id;
    const { plan_type, duration, status, selected_templates = [] } = req.body;
    ...
    if (plan_type !== "free") {
      if (!duration) {
        return res.status(400).json({ message: "duration is required for paid plans" });
      }

      endDate = new Date(startDate);

      if (duration === "1_month") endDate.setMonth(endDate.getMonth() + 1);
      if (duration === "3_months") endDate.setMonth(endDate.getMonth() + 3);
      if (duration === "6_months") endDate.setMonth(endDate.getMonth() + 6);
    }

    // ✅ If subscription exists → UPDATE
    if (existing) {
      const updated = await UserSubscription.query()
        .patchAndFetchById(existing.subscription_id, {
          plan_type,
